...
const BookCard=({book,setBooks}) => {
    ...
    const handleDelete=async()=>{
        try{
            ...
            toast.error("Failed to delte book");
        } finally {
            setShowModal(false);
        }
    };
    ...
    return(
        <>
        {/* CARD */}
        <Link to={`/book/${book._id}`} className={`relative block 
            rounded-xl bg-base-100 p-4 border transition-all duration-200 
        ...
        {showModal && (<dailog className="modal modal-open">
            <div className="modal-box">
                <h3 className="font-bold text-lg text-eeror flex items-center gap-2">
